	...
	local check = switch {
			...
			if (array.success) then
				if (functions.getTableCount(array.result) > 0) then
					...
					for i = 1, functions.getTableCount(array.result) do
						sendMessage(username, "#" .. array.result[i].id .. " (" .. array.result[i].status .. ") - " .. functions.truncate(array.result[i].description, 25))
					end
				else
		...
		["del"] = function()
			...
			if (args[3] ~= nil and args[3] ~= "" and type(tonumber(args[3])) == "number") then
				...
				local array = json.decode(jsonText)
				if (array.success) then
					sendMessage(username, data.lang.ticketDeleted)
				else
					sendMessage(username, data.error.wrongUser)
				end
			else
